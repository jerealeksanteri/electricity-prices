...
///
/// `echarts.min.js` loads asynchronously, so the first effect after hydration
/// may run before it exists — we retry until it's ready (this fixes charts that
/// only appeared after switching tabs). Resize is bound once per element.
